...
func _ready() -> void:
	if player_scene == null:
		player_scene = preload("res://soapbox.tscn")
...
func _build_track() -> void:
	...
	# Segments (auto-connected)
	var seg_size := Vector3(18, 1, 40)
	var seg_len := seg_size.z

	var b1 := Basis(Vector3(1, 0, 0), deg_to_rad(12.0))
	var p1 := Vector3(0, 6, 15)
	_make_static_box(track_root, "TrackSeg1", seg_size, p1, b1, mat)

	var b2 := Basis(Vector3(1, 0, 0), deg_to_rad(18.0))
	var p2 := _segment_end_pos(p1, b1, seg_len) + (b2 * Vector3(0, 0, seg_len * 0.5))
	_make_static_box(track_root, "TrackSeg2", seg_size, p2, b2, mat)

	var b3 := Basis(Vector3(1, 0, 0), deg_to_rad(12.0))
	var p3 := _segment_end_pos(p2, b2, seg_len) + (b3 * Vector3(0, 0, seg_len * 0.5))
	_make_static_box(track_root, "TrackSeg3", seg_size, p3, b3, mat)

	var b4 := Basis.IDENTITY
	var p4 := _segment_end_pos(p3, b3, seg_len) + (b4 * Vector3(0, 0, seg_len * 0.5))
	_make_static_box(track_root, "TrackSeg4", seg_size, p4, b4, mat)

	# Walls per segment + start/end caps
	...
	_add_walls_for_segment(walls, p4, b4, seg_size, wall_size, mat, "Seg4")

	var start_cap_pos := _segment_start_pos(p1, b1, seg_len)
	_add_end_cap(walls, "StartGate", start_cap_pos, b1, seg_size, 2.0, 4.0, mat)

	var end_cap_pos := _segment_end_pos(p4, b4, seg_len)
	_add_end_cap(walls, "EndGate", end_cap_pos, b4, seg_size, 2.0, 4.0, mat)

	# Finish trigger at end of seg4 (slightly before the wall feels good)
	_finish = Area3D.new()
	_finish.name = "Finish"
